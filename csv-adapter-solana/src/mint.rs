...
pub fn mint_right_from_hex_key(
    ...
    let key_bytes = hex::decode(cleaned)
        .map_err(|e| SolanaError::Wallet(format!("Invalid hex key: {}", e)))?;
    ...
    let secret_key: [u8; 32] = key_bytes.try_into()
    ...
    let blockhash_json: serde_json::Value = blockhash_resp.json()
        .map_err(|e| SolanaError::Rpc(format!("Failed to parse blockhash: {}", e)))?;
    
    let blockhash_str = blockhash_json.get("result")
        .and_then(|r| r.get("value").and_then(|v| v.get("blockhash").and_then(|b| b.as_str())))
    ...
    let instruction = Instruction::new_with_bytes(
        ...
        vec![
            ...
            AccountMeta::new_readonly(
                "11111111111111111111111111111111".parse::<Pubkey>().unwrap(),
                false
            ), // system program
        ],
    );
    ...
    let send_json: serde_json::Value = send_resp.json()
    ...
    let signature = send_json.get("result")
        .and_then(|r| r.as_str())
