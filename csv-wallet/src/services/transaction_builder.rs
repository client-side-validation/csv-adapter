...
async fn discover_sui_contracts(
    ...
    let response2 = client.post(rpc_url).json(&body2).send().await;

    if let Ok(resp) = response2 {
        if let Ok(json2) = resp.json::<serde_json::Value>().await {
            if let Some(data) = json2
                .get("result")
                .and_then(|r| r.get("data"))
                .and_then(|d| d.as_array())
            {
...
async fn discover_ethereum_contracts(
        ...
        if let Some(code) = json.get("result").and_then(|r| r.as_str()) {
            if code.len() > 2 && code != "0x" {
                contracts.push(DiscoveredContract {
                    ...
                    contract_type: ContractType::Unknown,
                    description: format!(
                        "Smart contract at address ({} bytes)",
                        (code.len() - 2) / 2
                    ),
                });
            }
    ...
    console::log_1(
        &format!(
            "Scanning Ethereum transactions for deployments by {}",
            address
        )
        .into(),
    );
            ...
            if let Some(hex_num) = json.get("result").and_then(|r| r.as_str()) {
                if let Ok(current_block) = u64::from_str_radix(&hex_num[2..], 16) {
                    ...
                    for block_num in start_block..=current_block {
                        ...
                        if let Ok(block_resp) =
                            client.post(rpc_url).json(&get_block_body).send().await
                        {
    ...
    if let Some(candidates) = candidate_contracts {
        console::log_1(
            &format!(
                "Verifying {} candidate Ethereum contracts",
                candidates.len()
            )
            .into(),
        );
        ...
        for candidate in candidates {
            ...
            if contracts
                .iter()
                .any(|c| c.address.to_lowercase() == candidate.to_lowercase())
            {
                continue;
            }
                ...
                if let Ok(json) = resp.json::<serde_json::Value>().await {
                    if let Some(code) = json.get("result").and_then(|r| r.as_str()) {
                        if code.len() > 2 && code != "0x" {
                            ...
                            console::log_1(
                                &format!("Verified Ethereum contract: {}", candidate).into(),
                            );
                            contracts.push(DiscoveredContract {
        ...
        Ok(resp) => {
            if let Ok(json) = resp.json::<serde_json::Value>().await {
                if let Some(accounts) = json.get("result").and_then(|r| r.as_array()) {
                    console::log_1(
                        &format!("Found {} program data accounts", accounts.len()).into(),
                    );
                                ...
                                if let Some(_data) = value.get("data").and_then(|d| d.as_array()) {
                                        ...
                                        .and_then(|p| p.get("info"))
                                    {
                                        if let Some(authority) =
                                            parsed.get("upgradeAuthority").and_then(|a| a.as_str())
                                        {
                                    ...
                                    if !is_owned_by_user {
                                        ...
                                        if let Some(program_data) = value
                                            .get("data")
                                            .and_then(|d| d.get("parsed"))
                                            .and_then(|p| p.get("info"))
                                            .and_then(|i| i.get("programData"))
                                            .and_then(|pd| pd.as_str())
                                        {
                                            ...
                                            if let Ok(data_resp) =
                                                client.post(rpc_url).json(&data_body).send().await
                                            {
                                                if let Ok(data_json) =
                                                    data_resp.json::<serde_json::Value>().await
                                                {
                                                    if let Some(data_value) = data_json
                                                        .get("result")
                                                        .and_then(|r| r.get("value"))
                                                    {
                                                        if let Some(data_parsed) = data_value
                                                            .get("data")
                                                            .and_then(|d| d.get("parsed"))
                                                            .and_then(|p| p.get("info"))
                                                        {
                                                            if let Some(authority) = data_parsed
                                    ...
                                    if is_owned_by_user {
                                        ...
                                        contracts.push(DiscoveredContract {
                                            ...
                                            contract_type: ContractType::Unknown,
                                            description:
                                                "Verified program (user is upgrade authority)"
                                                    .to_string(),
                                        });
                                    } else {
                                        // Still add it as a program if it exists
                                        console::log_1(
                                            &format!("Found Solana program: {}", candidate).into(),
                                        );
                                        contracts.push(DiscoveredContract {
                                            address: candidate.to_string(),
                                            contract_type: ContractType::Unknown,
                                            description: "Verified program (BPF Loader)"
                                                .to_string(),
                                        });
                                    }
                                } else {
