...
pub fn execute(action: WalletAction, config: &Config, state: &mut UnifiedStateManager) -> Result<()> {
    match action {
        WalletAction::Init {
            ...
            words,
            fund,
        } => cmd_init(network, words, fund, config, state),
...
fn save_wallet_config(
    mnemonic: &str,
    addresses: &std::collections::HashMap<Chain, String>,
    _config: &Config,
) -> Result<()> {
    use std::fs;
    use std::io::Write;
    ...
    match chain {
        Chain::Bitcoin => {
            if network == &Network::Test {
                output::info(&format!(
                    "Visit: https://signetfaucet.com/ to fund {}",
                    address
                ));
        ...
        Chain::Ethereum => {
            output::info(&format!(
                "Visit: https://faucet.sepolia.dev/ to fund {}",
                address
            ));
        }
        Chain::Sui => {
            output::info(&format!(
                "Visit: https://faucet.sui.io/ to fund {}",
                address
            ));
        }
        Chain::Aptos => {
            output::info(&format!(
                "Visit: https://faucet.testnet.aptoslabs.com/ to fund {}",
                address
            ));
        }
        Chain::Solana => {
