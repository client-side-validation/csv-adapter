//! Compile-fail test: ProofValidated to Locked (backward transition)
//!
//! This test ensures that going backward from ProofValidated to Locked
//! is impossible at compile time. Transitions must follow the state machine
//! forward direction only.

use csv_core::transfer_state::{Locked, ProofValidated, TransferData};
use csv_core::hash::Hash;
use csv_core::protocol_version::ChainId;

fn main() {
    let data = TransferData::new(
        Hash::new([1u8; 32]),
        csv_core::sanad::SanadId(Hash::new([2u8; 32])),
        ChainId::new("bitcoin"),
        ChainId::new("ethereum"),
        vec![3u8; 32],
        Hash::new([4u8; 32]),
    );

    let proof_validated = ProofValidated::new(data, vec![5u8; 32]);

    // This should fail to compile - cannot go backward from ProofValidated to Locked
    // The state machine only allows forward transitions.
    // Valid path: Locked -> AwaitingFinality -> ProofBuilding -> ProofValidated -> Minting -> Completed
    let _locked = Locked::new(proof_validated.data, 200, vec![6u8; 32]); // ERROR: backward transition not allowed
}
