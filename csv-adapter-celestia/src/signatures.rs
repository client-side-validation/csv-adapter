//! Celestia signature verification (ECDSA/secp256k1 Tendermint)
//!
//! Celestia uses Tendermint's ECDSA signatures over secp256k1.
...
/// Verify a Celestia Tendermint ECDSA signature
pub fn verify_celestia_signature(
    signature: &[u8],
    public_key: &[u8],
    message: &[u8],
) -> Result<()> {
    ...
    if public_key.len() != 33 {
        return Err(AdapterError::SignatureVerificationFailed(
            format!("Invalid public key length: {} (expected 33)", public_key.len())
        ));
    }
...
pub fn verify_celestia_signatures(
    ...
    for (i, (sig, pk, msg)) in signatures.iter().enumerate() {
        verify_celestia_signature(sig, pk, msg).map_err(|e| {
...
mod tests {
    ...
    #[test]
    fn test_valid_celestia_signature() {
    ...
    fn test_tampered_signature() {
        ...
        sig[0] ^= 0xFF;
        assert!(verify_celestia_signature(&sig, &pk, &msg).is_err());
    }
}
